...
// /teachings/talks always win over this dynamic segment.
...
export default async function TeachingPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (post.category === "TALK") redirect(`/teachings/talks/${post.slug}`);
  if (post.category === "DANCE_ARTICLE") redirect(`/teachings/dances/articles/${post.slug}`);
