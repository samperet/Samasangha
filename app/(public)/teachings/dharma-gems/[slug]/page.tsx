...
export async function generateMetadata(
  ...
  return { title: post?.title ?? "Dharma Gem" };
}

export default async function DharmaGemPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const post = await prisma.post.findUnique({
    where: { slug, category: "DHARMA_GEM", published: true },
  }).catch(() => null);
